  ...
  return (
    <section id="skills" className="py-24 md:py-32 relative overflow-hidden" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={cn(
          "mb-12 md:mb-20 transition-all duration-1000",
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
        )}>
          <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold tracking-tight mb-6">
            Built on a foundation of <span className="text-primary">technical excellence.</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {skillCategories.map((category, idx) => (
            <div 
              key={category.title} 
              className={cn(
                "p-6 sm:p-10 rounded-2xl md:rounded-[2.5rem] glass-card hover-glow",
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              )}
              style={{ transitionDelay: `${idx * 100}ms` }}
            >
              <div className="flex items-center space-x-4 sm:space-x-6 mb-8 md:mb-10">
                <div className="p-3 sm:p-4 rounded-xl md:rounded-2xl bg-primary/10 text-primary">
                  <category.icon className="h-6 w-6 sm:h-7 sm:w-7" />
                </div>
                <h3 className="font-bold text-xl sm:text-2xl">{category.title}</h3>
              </div>
        ...
        <div className="mt-16 md:mt-20 pt-12 md:pt-16 border-t border-white/5">
          <div className="flex flex-wrap justify-center gap-3 md:gap-4">
            {["Git", "AWS", "Linux", "MLflow", "PyTorch", "TensorFlow", "ROS2", "Docker", "NVIDIA CUDA"].map((tool, idx) => (
              <div 
                key={tool} 
                className="px-5 py-2.5 md:px-8 md:py-4 rounded-full glass-card border-white/5 text-[10px] md:text-sm font-bold hover:bg-white/5 transition-all cursor-default"
